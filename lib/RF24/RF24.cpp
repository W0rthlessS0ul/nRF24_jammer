...
/*
 * Modified RF24 library version optimized for jamming applications
 * 
 * Credits:
 * - Original library: https://github.com/nRF24/RF24 
 * - Modifications by: W0rthlessS0ul
 * 
 * This is NOT the original work - just my jammer-friendly adaptation.
 * 
 * License: MIT (use freely, credits appreciated but not mandatory).
 * 
 * Unlike some overzealous folks who file "Copyright Infringement",
 * I won't bother you with legal threats - you're free to use this
 * however you want. Though I'll genuinely appreciate if you keep 
 * this credit notice somewhere visible.
 * 
 * P.S. Major thanks to the original authors for their great work!
 */
...
void RF24::csn(bool mode) //New modified code
{
    digitalWrite(csn_pin, mode);
}
/*void RF24::csn(bool mode) //Old code
{
...
inline void RF24::beginTransaction() //New modified code
{
    csn(LOW);
}
/*inline void RF24::beginTransaction() //Old code
{
...
inline void RF24::endTransaction() //New modified code
{
    csn(HIGH);
}
/*inline void RF24::endTransaction() //Old code
{
    csn(HIGH);
#if defined(RF24_SPI_TRANSACTIONS)
    #if defined(RF24_SPI_PTR)
...
uint8_t RF24::read_register(uint8_t reg) //New modified code
{
    beginTransaction();
    status = _spi->transfer(reg);
...
/*uint8_t RF24::read_register(uint8_t reg) //Old code
{
    uint8_t result;
...
void RF24::write_register(uint8_t reg, uint8_t value) //New modified code
{
    beginTransaction();
    status = _spi->transfer(W_REGISTER | reg);
    _spi->transfer(value);
...
/*void RF24::write_register(uint8_t reg, uint8_t value) //Old code
{
...
void RF24::setChannel(uint8_t channel) //New modified code
{
    beginTransaction();
    status = _spi->transfer(W_REGISTER | RF_CH);
    _spi->transfer(channel);
    endTransaction();
}
/*void RF24::setChannel(uint8_t channel) //Old code
{
...
void RF24::startConstCarrier(rf24_pa_dbm_e level, uint8_t channel) //New modified code
{
...
/*void RF24::startConstCarrier(rf24_pa_dbm_e level, uint8_t channel) //Old code
{
    stopListening();
    write_register(RF_SETUP, read_register(RF_SETUP) | _BV(CONT_WAVE) | _BV(PLL_LOCK));
    if (isPVariant()) {
...
void RF24::stopConstCarrier() //New modified code
{
    ce(LOW);
    write_register(RF_SETUP, read_register(RF_SETUP) & ~_BV(CONT_WAVE) & ~_BV(PLL_LOCK));
}
/*void RF24::stopConstCarrier() //Old code
{
