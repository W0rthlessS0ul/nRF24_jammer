...
void updateDisplay(int menuNum)
{
    ...
    const uint8_t *bitmap = (menu_number == 0) ? bitmap_bluetooth_jammer : (menu_number == 1) ? bitmap_drone_jammer
...
void setup()
{
    ...
    if (access_point == 0)
    {
        ...
        registerRoute("/OTA", []()
                      { settingsHandler(html_ota); });
        registerRoute("/wifi_select", []()
                      { settingsHandler(html_wifi_select); });
        ...
        server.on("/update", HTTP_POST, []()
                  { server.send(200, "text/plain", (Update.end() ? "Update Success" : "Update Failed")); }, handleFileUpload);
        ...
        registerRoute("/logo_on", []()
                      { storeEEPROMAndSet(7, 0, logo); });
        registerRoute("/logo_off", []()
                      { storeEEPROMAndSet(7, 1, logo); });
        ...
        registerRoute("/access_point_off", []()
                      { access_poin_off(); });
...
void executeAction(int menuNum)
{
    if (menuNum == 5)
        misc();
    display.clearDisplay();
    const uint8_t *bitmap = (menu_number == 0) ? bitmap_bluetooth_jam : (menu_number == 1) ? bitmap_drone_jam
