...
// ----------------------------------------------------------------
// HSPI_init / HSPI_deinit
// Sul LOLIN32 Lite si usa VSPI (bus SPI hardware predefinito ESP32).
// Pin: SCK=18  MISO=19  MOSI=23  (CE/CSN gestiti da RF24)
// Frequenza ridotta a 10 MHz per stabilità con PA+LNA.
// LED su GPIO22, ACTIVE-LOW: LOW=acceso, HIGH=spento.
// ----------------------------------------------------------------
void HSPI_init()
{
  // VSPI: SCK=18, MISO=19, MOSI=23, SS=-1 (gestito da RF24 tramite CSN)
  hp = new SPIClass(VSPI);
  hp->begin(NRF_SCK, NRF_MISO, NRF_MOSI, -1);
  hp->setFrequency(10000000);   // 10 MHz – più stabile con PA+LNA
  ...
  // LED LOLIN32 Lite: GPIO22 ACTIVE-LOW → LOW = acceso
...
void HSPI_deinit()
{
  ...
  // LED LOLIN32 Lite: GPIO22 ACTIVE-LOW → HIGH = spento
