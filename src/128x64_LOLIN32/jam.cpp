// ============================================================
// jam.cpp - Adattato per LOLIN32 Lite
// Differenze rispetto all'originale:
//   - SPIClass(VSPI) invece di SPIClass(HSPI)
//   - hp->begin(NRF_SCK, NRF_MISO, NRF_MOSI, -1) con pin espliciti
//   - Frequenza SPI 10MHz (piu stabile con PA+LNA)
//   - LED_PIN / LED_ON / LED_OFF (active LOW)
// ============================================================
#include "jam.h"
...
void HSPI_init()
{
  // LOLIN32 Lite usa VSPI (GPIO 18=SCK, 19=MISO, 23=MOSI)
  hp = new SPIClass(VSPI);
  hp->begin(NRF_SCK, NRF_MISO, NRF_MOSI, -1);
  // 10 MHz: piu stabile con il modulo PA+LNA ad alta corrente
  hp->setFrequency(10000000);
  ...
  for ( int i = 0; i < nrf24_count; i++ )
  {
    ...
    switch ( nrf_pa )
    {
    case 0: nrf_pa_level = RF24_PA_MAX;  break;
    case 1: nrf_pa_level = RF24_PA_HIGH; break;
    case 2: nrf_pa_level = RF24_PA_LOW;  break;
    case 3: nrf_pa_level = RF24_PA_MIN;  break;
    default: nrf_pa_level = RF24_PA_MAX; break;
    }
  ...
  // Accende LED (ACTIVE LOW su LOLIN32 Lite)
...
void wifi_jam()
{
  InitRadios();
  HSPI_init();
  btnNext.tick(); btnOK.tick(); btnPrevious.tick();
  bool SerialStop = true;
  while ( (!btnNext.isSingle() && !btnPrevious.isSingle() &&
           !btnOK.isDouble() && !btnOK.isSingle()) && SerialStop )
  {
      ...
      if ( Separate_or_together == 0 )
      {
        ...
        for ( int j = 0; j < nrf24_count; j++ )
        {
          ...
          for ( int i = 0; i < count; i++, ch++ )
          {
            btnOK.tick();
            SerialStop = SerialCommands();
            if ( btnOK.isSingle() || !SerialStop ) break;
            radios[j]->setChannel(ch);
            radios[j]->writeFast(&jam_text, sizeof(jam_text));
          }
        }
