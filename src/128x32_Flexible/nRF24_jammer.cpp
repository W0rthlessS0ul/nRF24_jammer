    ...
    if ( buttons == 0 )
    {
      ...
      if ( btnOK.isHolded() )
      {
        if ( menu_number == 0 )
        {
          ...
          {
            jamHandler(wifi_jam, String("WiFi Jam"), bitmap_wifi_jam, true);
            if ( btnOK.isSingle() )
              break;
        ...
        else if ( menu_number == 2 )
        {
          ...
          while ( !btnOK.isSingle() )
          {
            jamHandler(wifi_scan_jam, String("WiFi Jam"), bitmap_wifi_jam, true);
            if ( btnOK.isSingle() )
              break;
...
void setup()
{
  ...
  if ( access_point == 0 )
  {
    ...
    registerRoute("/bluetooth_jam", []() { jamHandler(bluetooth_jam, String("Bluetooth Jamming"), bitmap_bluetooth_jam, false); });
    registerRoute("/drone_jam", []() { jamHandler(drone_jam, String("Drone Jamming"), bitmap_drone_jam, false); });
    registerRoute("/wifi_jam", []() { jamHandler(wifi_jam, String("WiFi Jamming"), bitmap_wifi_jam, false); });
    registerRoute("/wifi_deauth_all", []() { attackHandler(String("WiFi Deauthing"), wifi_deauth_all, bitmap_wifi_deauth); });
    registerRoute("/wifi_scan_jam", []() { jamHandler(wifi_scan_jam, String("WiFi Jamming"), bitmap_wifi_jam, false, false, true); });
    ...
    registerRoute("/ble_advertising_jam", []() { jamHandler(ble_advertising_jam, String("BLE Jamming"), bitmap_ble_jam, false); });
    registerRoute("/ble_data_jam", []() { jamHandler(ble_data_jam, String("BLE Jamming"), bitmap_ble_jam, false); });
    registerRoute("/zigbee_jam", []() { jamHandler(zigbee_jam, String("Zigbee Jamming"), bitmap_zigbee_jam, false); });
